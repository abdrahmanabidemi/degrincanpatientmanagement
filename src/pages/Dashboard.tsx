...
export default function Dashboard() {
  ...
  const load = async () => {
    ...
    if (!c && isAdmin) {
      const { data: created, error: createErr } = await supabase
        ...
        .insert({
          user_id: user.id,
          clinic_name: "DE' GRINCAN HEALTHCARE",
          email: user.email ?? "degrincanhealthcare@gmail.com",
        } as never)
        .select()
        .maybeSingle();
      if (createErr) {
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    const rawName = (clinic?.clinic_name ?? "DE' GRINCAN HEALTHCARE").trim();
    const safeName = rawName.replace(/[\\/:*?"<>|\x00-\x1F]/g, "").replace(/\s+/g, " ").trim();
    a.download = safeName ? `${safeName} - Patients.csv` : `DE' GRINCAN HEALTHCARE Patients.csv`;
  ...
  const contact = (p: Patient) => {
    ...
    const clinicName = (clinic?.clinic_name ?? "DE' GRINCAN HEALTHCARE").trim();
  ...
  const headerName = clinic?.clinic_name ?? "DE' GRINCAN HEALTHCARE";
