...
export default function ResetPassword() {
  ...
  useEffect(() => {
    // Check for errors in the hash fragment (e.g. from expired invitation links)
    const hash = window.location.hash || "";
    if (hash.includes("error_description=")) {
      const match = /error_description=([^&]+)/.exec(hash);
      if (match) {
        setErrorMsg(decodeURIComponent(match[1].replace(/\+/g, " ")));
        return;
      }
    }
    ...
    // Fallback timer: if no valid session is resolved within 4 seconds, show an error
    let timer = setTimeout(() => {
      setErrorMsg("This invitation or recovery link has expired, is invalid, or was already used.");
    }, 4000);
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-soft">
      <div className="w-full max-w-md">
        <Link to="/" className="flex flex-col items-center justify-center gap-2 mb-8">
          ...
          <span className="font-display font-bold text-2xl text-center">DE' GRINCAN HEALTHCARE</span>
        </Link>
